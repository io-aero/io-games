...
def draw():
    ...
    if game_over: display_message("GAME OVER!", "try agian.")
    elif game_complete:
...
def animate_stars(stars_to_animate):
    for star in stars_to_animate:
        ...
        aniamation = animate(star, duration=duration, on_finished=handle_game_over, y=HEIGHT) # type: ignore
        aniamations.append(aniamation)
...
def red_star_click():
    global current_level, stars, aniamations, game_complete
    stop_animations(aniamations)
...
def display_message(headin_text, sub_heading_text):
    screen.draw.text(headin_text, fontsize=60, center=CENTER, color=FONT_COLOR)  # type: ignore
    screen.draw.text(sub_heading_text, # type: ignore
                    fontsize=30,
