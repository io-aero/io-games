...
rem ----------------------------------------------------------------------------
rem
rem run_io_games_dev.bat: Process IO-GAMES tasks.
rem
rem ----------------------------------------------------------------------------
...
set IO_AERO_GAMES_LOG=run_io_games_dev_%IO_AERO_TASK%.log
...
> %IO_AERO_GAMES_LOG% 2>&1 (
    ...
    if /I ["%IO_AERO_TASK%"] EQU ["version"] (
        ...
        if ERRORLEVEL 1 (
            echo Processing of the script run_io_games_dev was aborted
            exit 1
        )
...
echo Processing of the script run_io_games_dev is aborted: unknown task='%IO_AERO_TASK%'
exit 1
